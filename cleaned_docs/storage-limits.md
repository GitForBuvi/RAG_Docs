...
If VolumeLimitScaling feature gate is enabled and a CSI driver has corresponding CSIDriver object installed with spec.preventPodSchedulingIfMissing set to true then scheduler will prevent pod placement to nodes that do not yet have CSI driver installed.  For exmaple:
yaml
apiVersion: storage.k8s.io/v1
kind: CSIDriver
metadata:
