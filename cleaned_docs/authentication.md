...
See Managing Certificates for how to generate a client cert, or read the brief example later in this page.
...
{{< note >}}
Kubernetes {{< skew currentVersion >}} does not support certificate revocation.
...
and the AllowParsingUserUIDFromCertAuth feature gate
...
{{< /warning >}}
External integrations
Kubernetes has native support for JWT and for OpenID Connect (OIDC); see JSON Web Token authentication.
...
{{% /tab %}}
{{% tab name="authentication.k8s.io/v1beta1" %}}yaml
{
...
See web request header configuration to learn about
...
command line arguments, or using a
configuration file.
...
: This special command line argument specifies that you want to configure authentication using a configuration file.
...
{{% /tab %}}
{{% tab name="client.authentication.k8s.io/v1beta1" %}}yaml
apiVersion: v1
kind: Config
users:
- name: my-user
