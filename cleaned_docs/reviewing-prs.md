...
Read the  content guide and
  style guide so you can leave informed comments.
...
flowchart LR
    ...
    N --> O[new contributors shouldchoose Comment]
    end
    subgraph third[Select PR]
    direction TB
    T[ ] -.-
    J[read descriptionand comments]--> K[preview changes inNetlify preview build]
    end
A[Review open PR list]--> B[Filter open PRsby label]
  B --> third --> fourth
...
Go to https://github.com/kubernetes/website/pulls.
...
Before reviewing blog PRs, be familiar with the blog guidelines
  and with submitting blog posts and case studies.
