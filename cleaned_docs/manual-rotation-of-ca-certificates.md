...
Update the --root-ca-file flag for the {{< glossary_tooltip term_id="kube-controller-manager" >}} to include
...
Update the --root-ca-file flag for the {{< glossary_tooltip term_id="cloud-controller-manager" >}} to include
...
{{< note >}}
  ...
  see Certificates (openssl).
  You can also use cfssl.
  {{< /note >}}
...
On each node, restart the kubelet by removing the old CA from file against the clientCAFile flag
